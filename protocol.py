...
class VideoHeader:
    ...
    def pack(self) -> bytes:
        ...
        return struct.pack(
            ...
            self.session_id if isinstance(self.session_id, bytes) else self.session_id.encode(),
            self.total_frames,
            self.fps,
            output_bytes
        )
...
class Message:
    """通用消息容器"""
    header: VideoHeader | AudioHeader
    ...
    def is_video(self) -> bool:
        return isinstance(self.header, VideoHeader)
...
class ProtocolParser:
    ...
    def parse(data: bytes) -> Optional[Message]:
        ...
        if msg_type == MessageType.AUDIO_DATA:
            ...
            payload = data[AUDIO_HEADER_SIZE:] if len(data) > AUDIO_HEADER_SIZE else None
        else:
            if len(data) < VIDEO_HEADER_SIZE:
                return None
            header = VideoHeader.unpack(data)
            payload = data[VIDEO_HEADER_SIZE:] if len(data) > VIDEO_HEADER_SIZE else None
    ...
    def _parse_legacy(data: bytes) -> Optional[Message]:
        ...
        if msg_type == MessageType.AUDIO_DATA and len(data) >= 64:
            try:
                ...
                return Message(header=header, data=data[64:] if len(data) > 64 else None)
            except:
                return None
        ...
        if len(data) >= 128:
            try:
                ...
                return Message(header=header, data=data[128:] if len(data) > 128 else None)
            except:
                return None
