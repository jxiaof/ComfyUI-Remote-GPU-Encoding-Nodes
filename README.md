...
*基于 RGB uint8 tensor 原始数据（未压缩）*

原始视频帧的计算：
- 720p = 1280x720 像素
- 1080p = 1920x1080 像素
- 4K = 3840x2160 像素
假设是 RGB（3通道）或 RGBA（4通道），每个通道 8位（1字节）或 16位（2字节）或 32位浮点（4字节）。
常见的 tensor 格式：
1. uint8 (0-255) - 每个像素 1 字节
2. uint16 - 每个像素 2 字节
3. float32 - 每个像素 4 字节
RGB uint8:
- 720p: 1280 * 720 * 3 = 2,764,800 字节 ≈ 2.63 MB
- 1080p: 1920 * 1080 * 3 = 6,220,800 字节 ≈ 5.93 MB
- 4K: 3840 * 2160 * 3 = 24,883,200 字节 ≈ 23.73 MB
RGB uint16:
- 720p: 1280 * 720 * 3 * 2 = 5,529,600 字节 ≈ 5.27 MB
- 1080p: 1920 * 1080 * 3 * 2 = 12,441,600 字节 ≈ 11.86 MB
- 4K: 3840 * 2160 * 3 * 2 = 49,766,400 字节 ≈ 47.48 MB
RGB float32:
...
- **带宽**: 建议 10Gbps 或更高（4K@60fps 需要 11.4 Gbps）
- **延迟**: 建议 <5ms（局域网）
- **稳定性**: 丢包率 <0.1%
